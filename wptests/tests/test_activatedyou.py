...
from ..pages.activatedyou import *
from ..products.activatedyou_products import PRODUCTS
from pytest_testrail.plugin import pytestrail


@pytest.mark.ui
class TestActivatedYou:  # Must start with 'Test...'
    ...
    def test_header(self, browser, app_config):
        ...
        header.go(app_config.base_url_activatedyou)
        # logo
        header.click_on_link(header.logo_link)
        # home
        header.click_on_link(header.home_link)
    ...
    def test_ftc_signup(self, browser, app_config):
        ...
        shop_page.go(app_config.base_url_activatedyou)
        shop_page.ftc_signup('', 'tester@goldenhippo.com')
        assert not ftc_page.is_on_page()
        shop_page.ftc_signup('Tester', '')
        assert not ftc_page.is_on_page()
        shop_page.ftc_signup('Tester', 'tester@goldenhippo.com')
    ...
    def test_ftc_products_page(self, browser, app_config):
        ...
        """
        ftc_page = FTCPage(driver=browser)
        ftc_page.go(app_config.base_url_activatedyou)
    ...
    def test_submit_contact_form(self, browser, app_config):
        ...
        contact_page.go(app_config.base_url_activatedyou)
    ...
    def test_get_products(self, browser, app_config):
        ...
        products_page.go(app_config.base_url_activatedyou)
    ...
    def test_add_to_cart(self, browser, app_config, title, page, quantity):
        ...
        cart_page = CartPage(driver=browser)
        url = app_config.base_url_activatedyou + page
        product_page.go(url)
    ...
    def test_cart_increase_quantity(elf, browser, app_config, title, page, quantity):
        ...
        product_page = ProductDetailPage(driver=browser)
        cart_page = CartPage(driver=browser)
        url = app_config.base_url_activatedyou + '/supplements/' + page
        product_page.go(url)
        product_page.select_quantity(quantity)
        pp_price = product_page.price
        product_page.click_addtocart()
    ...
    def test_coupon_code(self, browser, app_config):
        ...
        """
        cart_page = CartPage(driver=browser)
        url = app_config.base_url_activatedyou + '/cmd.php?pid=f7d1003f4829438e80ffa60c416df476'
        cart_page.go(url)
